# subracer.pipeline.orchestrator
#
...
class Orchestrator:
  ...
  #    subracer processes can't clobber the same TSVs. By default it keeps running
  ...
  def run(
    ...
    if not lock.acquire():
      raise RunLockError(f"Another subracer run is already active for {target_dir}")
    try:
  ...
  #    state_dir [Path]:                the .subracer state directory
