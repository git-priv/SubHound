# subracer.pipeline.lock
#
# A cross-platform, advisory single-run lock so two subracer processes can't work
...
class RunLockError(RuntimeError):
  # Raised when the lock is already held by another subracer process.
  pass
...
class RunLock:
  ...
  #
  # Example:
  #    RunLock(Path("/media/.subracer/lock"))
  ...
  def __enter__(self) -> RunLock:
    if not self.acquire():
      raise RunLockError(f"Another subracer run is already active ({self.path})")
    return self
