# subracer.scheduling
#
# Helpers to run subracer periodically via the OS scheduler (cron on Linux/macOS,
...
# day) is left, exit and let the OS restart subracer on a schedule. Each run
...
# Marker comment tagging the crontab line subracer manages, so it can be replaced
# or removed without touching the user's other cron entries.
CRON_MARKER = "# subracer-auto"
TASK_NAME = "subracer"
...
#
# Example:
#    build_run_command(Path("/media"), ["en"])  ->  [".../python", "-m", "subracer", "--headless", ...]
def build_run_command(
  target_dir: Path,
  languages: list[str],
  python: str | None = None,
) -> list[str]:
  cmd = [python or sys.executable, "-m", "subracer", "--headless", "--once",
         "--dir", str(target_dir)]
  if languages:
...
# Function Summary:
#    Produce the exact scheduler entry that would run subracer periodically, for
...
#      ->  "0 * * * * /usr/bin/python -m subracer --headless --dir /media --languages en  # subracer-auto"
...
# Function Summary:
#    Install (or replace) the periodic subracer schedule on this machine. On
#    Linux/macOS it rewrites the user's crontab, preserving every line except a
#    previous subracer entry; on Windows it creates/replaces the scheduled task.
...
def install_schedule(
  ...
  # cron (Linux/macOS): rewrite the crontab, keeping all non-subracer lines.
  try:
