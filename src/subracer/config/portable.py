# subracer.config.portable
#
# Portable export / import of subracer configuration, like a password manager's
...
BUNDLE_VERSION = 1
_HEADER = (
  "# subracer configuration export\n"
  "# !!! PLAINTEXT - THIS FILE CONTAINS YOUR CREDENTIALS IN THE CLEAR !!!\n"
  "# Import it on the target machine, then DELETE this file (subracer can shred\n"
...
#
# Example:
#    export_bundle(Settings(), Credentials(api_key="k"), Path("/tmp/subracer.toml"))
#      ->  PosixPath("/tmp/subracer.toml")
def export_bundle(
  settings: Settings,
  creds: Credentials,
  out_path: Path,
  ...
  payload: dict = {
    "subracer_export_version": BUNDLE_VERSION,
...
#
# Example:
#    read_bundle(Path("/tmp/subracer.toml"))  ->  (Settings(...), Credentials(...))
...
#    config_directory [Path]:  the subracer config directory to install into
#
#  Output:
#    installed [tuple[Settings, Credentials]]:  what was installed
#
# Example:
#    install_bundle(Path("/tmp/subracer.toml"), Path("~/.config/subracer"))
...
#
# Example:
#    delete_file(Path("/tmp/subracer.toml"))  ->  True
